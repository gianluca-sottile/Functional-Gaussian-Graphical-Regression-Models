...
for(h in seq_len(nsim)) {
  ...
  for(j in 1:nrho) {
    out.lambda <- jcglasso(data = data.listYX, rho = rho.seq[j], lambda = 1E12,
                           alpha1 = 0.0, alpha2 = 0.0, alpha3 = 0.0, nu = 0.0)
    lambda.seq <- lmbMax_fun(out.lambda) * perc.lmbd.seq
    out.tht <- jcglasso(data = data.listYX, rho = rho.seq[j], lambda = lambda.seq,
                        alpha1 = 0.0, alpha2 = 0.0, alpha3 = 0.0, nu = 0.0, trace = 0L)
    
    thetah <- 0.0
    b.stim <- tht.stimYX <- tht.stim <- sig.stim <- B.stim <- vector(mode = "list", length = L)
    Bmatrix <- array(0.0, dim = c(q + 1, p, L, nlambda))
    THTmatrix <- array(0.0, dim = c(p, p, L, nlambda))
    for(l in seq_len(L)) {
    ...
    tmp <- ROCB(Bh = 1 * (b.stim[[1]] != 0), Bt = B.supp, nlambda = nlambda, plot.it = FALSE)
    # print(tmp)
    tmp2 <- sapply(1:nlambda, \(r) mean(sapply(1:L, \(l) norm(Par$B[, , l] - Bmatrix[-1, , l, r], "F"))))
  ...
  b.stim <- array(0.0, dim = c(q, p, nlambda, L, nrho))
  B.stim <- array(0.0, dim = c(p, q, nlambda, L, nrho))
  thetah <- array(0.0, dim = c(p, p, nlambda, L, nrho))
  tht.stimYX <- array(0.0, dim = c(p + q, p + q, nlambda, L, nrho))
  sig.stim <- tht.stim <- array(0.0, dim = c(p, p, nlambda, L, nrho))
  Bmatrix <- array(0.0, dim = c(q + 1, p, L, nlambda, nrho))
  THTmatrix <- array(0.0, dim = c(p, p, L, nlambda, nrho))
  for(k in 1:L) {
    ...
    rho.seq <- rhoMax_fun(outCGL) * perc.rho.seq
    for(j in 1:nrho){
      outCGL2 <- jcglasso(data = data.listYXk, rho = rho.seq[j], lambda = 1E12, nu = 0.0, 
                          alpha1 = 0.0, alpha2 = 0.0, alpha3 = 0.0)  
      lambda.seq <- lmbMax_fun(outCGL2) * perc.lmbd.seq
      outCGL3 <- jcglasso(data = data.listYXk, rho = rho.seq[j], lambda = lambda.seq, nu = 0.0, 
  ...
  tmp <- sapply(1:nrho, \(j) {
    bh <- 1 * ((b.stim[, , , 1, j] != 0) | (b.stim[, , , 2, j] != 0) | (b.stim[, , , 3, j] != 0))
    ROCB(Bh = bh, Bt = B.supp, nlambda = nlambda, plot.it = FALSE)$ROC
  })
  
  tmp2 <- sapply(1:nlambda, \(j) {
    sapply(1:nrho, \(r) mean(sapply(1:L, \(l) norm(Par$B[, , l] - Bmatrix[-1, , l, r, j], "F"))))
  })
...
dfAUC <- data.frame(value = c(colMeans(resAUC), colMeans(resAUC3)), 
                    rho = rep(round(perc.rho.seq, 2), 2L),
                    method = factor(rep(c("fGGRM", "Naive"), each = nlambda), 
                                    levels = c("fGGRM", "Naive")))

p1 <- ggplot(dfAUC) + 
  geom_point(aes(x = rho, y = value, shape = method), size = 3) +
  geom_line(aes(x = rho, y = value, linetype = method)) +
  theme_classic() + ylab(expression(Area~under~the~curve~of~hat(B))) + xlab(expression(rho / rho[max])) + 
  theme(axis.title = element_text(size = 14), axis.text = element_text(size = 12),
        axis.text.x = element_text(angle = 45, vjust = .4), 
        legend.text = element_text(size = 14), strip.text = element_text(size = 12),
        legend.position = "inside", legend.title = element_blank(), legend.position.inside = c(.8,.4))
p1

dfMSE2 <- data.frame(value = c(c(apply(resMSE, 3, colMeans)), c(apply(resMSE3, 3, colMeans))), 
                     rho = factor(rep(rep(round(perc.rho.seq, 2), each = nlambda), 2L), 
                                  levels = round(rev(perc.rho.seq), 2),
                                  labels = c(expression(rho / rho[max] == 0.1),
                                             expression(rho / rho[max] == 0.2),
                                             expression(rho / rho[max] == 0.3),
                                             expression(rho / rho[max] == 0.4),
                                             expression(rho / rho[max] == 0.5),
                                             expression(rho / rho[max] == 0.6),
                                             expression(rho / rho[max] == 0.7),
                                             expression(rho / rho[max] == 0.8),
                                             expression(rho / rho[max] == 0.9),
                                             expression(rho / rho[max] == 1.0))),
                     nu = rep(rep(round(perc.lmbd.seq, 2), nrho), 2L),
                     method = factor(rep(c("fGGRM", "Naive"), each = nlambda*nrho), 
                                     levels = c("fGGRM", "Naive")))

p2bis <- ggplot(subset(dfMSE2, rho %in% c(expression(rho / rho[max] == 0.2),
                                          expression(rho / rho[max] == 0.4),
                                          expression(rho / rho[max] == 0.6),
                                          expression(rho / rho[max] == 0.8)))) + 
  ...
  theme_classic() + ylab(expression(Mean~Squared~Error~of~hat(B))) + xlab(expression(nu / nu[max])) + 
  theme(axis.title = element_text(size = 14), axis.text = element_text(size = 12),
        axis.text.x = element_text(angle = 45, vjust = .4), 
        legend.text = element_text(size = 14), strip.text = element_text(size = 12),
...
ggsave("~/Downloads/AUC_b_n100.pdf", plot = p1, device = "pdf", scale = .7, width = 12, height = 10, units = "in")
ggsave("~/Downloads/MSE_b_n100.pdf", plot = p2bis, device = "pdf", scale = .7, width = 12, height = 10, units = "in")

gridExtra::grid.arrange(p1, p2bis, nrow = 1L)

# par(mfrow=c(1,2))
# matplot(perc.rho.seq, t(apply(resMIN[, , 1, ], 2, colMeans)), type = "l")
# matplot(perc.rho.seq, t(apply(resMIN[, , 2, ], 2, colMeans)), type = "l")
